...
std::vector<std::string> read_weather_file_vRaw(const std::string& path, const std::string& type_forecast)
{
	...
	if (!fin.is_open()) {
		std::cout << "Файл для проверки структуры скаченого файа с прогнозом "<< type_forecast <<" не найден." << std::endl;
		std::cout << "Для работы необходим эталонный файл STRUCT_"<< type_forecast <<".txt" << std::endl;
		exit(EXIT_FAILURE);
	}
	std::string Struct_etalon;
	std::getline(fin, Struct_etalon);
	fin.close();
	...
	if (m_vRawStruct[0] == "No errors" && m_vRawStruct[1] == "No warnings" && m_vRawStruct[5] == Struct_etalon){
	...
	else
	{
		...
		if (m_vRawStruct[5] != Struct_etalon) { std::cout << "The decoding structure is not correct. \nNeed:\n" << Struct_etalon << "\n\n"; };
		exit(EXIT_FAILURE);
	}
	return vRaw_Element;
}
